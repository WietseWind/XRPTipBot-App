...
class SendView extends Component {
    ...
    keyboardDidShow = () => {
        Animated.parallel([
            ...
            Animated.spring(
                this.KeyboardHeight,
                {
                    toValue: (AppSizes.screen.height * 0.43 + AppSizes.screen.height * 0.08)  + ( Platform.OS === "android" ? 80 : 50 )
                }
            )
        ]).start();
    };
    ...
    sendTip = () => {
        ...
        this.props.tip(sendAmount, `xrptipbot://${sendTo.network}/${sendTo.username}`).then((slug) => {
            this.setState({
                step: 'success',
                to: slug
            });
    ...
    renderPicker = () => {
        ...
        if (sendTo){
            ...
            switch (sendTo.network) {
                case "twitter" :
                    tintColor = "#1DA1F2";
                    networkIcon = <Image style={[iconStyle, {tintColor} ]} source={require('../../assets/images/twitter.png')} />;
                    break;
                case "reddit":
                    tintColor = "#ff4500";
                    networkIcon = <Image style={[iconStyle, {tintColor} ]} source={require('../../assets/images/reddit.png')} />;
                    break;
                case "discord":
                    tintColor = "#7289DA";
                    networkIcon = <Image  style={[iconStyle, {tintColor} ]} source={require('../../assets/images/discord.png')} />;
                    break;
                case "internal":
            ...
            return(
                <View
                    style={[styles.pickerContainer, AppStyles.centerAligned]}
                >
                    <View style={[AppStyles.flex1, {paddingLeft: 15}]}>
                        {networkIcon}
                    </View>
                    <View style={[AppStyles.flex4, AppStyles.centerAligned]}>
                        <Text style={[AppStyles.baseText, AppStyles.strong]}>{sendTo.network === "discord" || sendTo.network === "internal" ? sendTo.slug : sendTo.username}</Text>
                    </View>
                    <TouchableOpacity onPress={() => {this.setState({sendTo: null})}} style={[AppStyles.flex1, {alignItems: "flex-end", paddingRight: 15}]}>
                ...
                >
                    ...
                    >
                        <View style={[styles.textContainer]}>
                            ...
                            }]} resizeMode={'stretch'} source={require('../../assets/images/contacts.png')}/>
                        </View>
                    </TouchableHighlight>
                    <TouchableHighlight
                        underlayColor={'#ffffff'}
                    ...
                    >
                        <View style={[styles.textContainer]}>
                            ...
                            }]} resizeMode={'stretch'} source={require('../../assets/images/qr.png')}/>
                        </View>
                    </TouchableHighlight>
    ...
    renderKeyboardHeader = () => {
        ...
        let mustTips = _.sortBy(accountState.mustTip || [], [function(o) { return o.last_use; }]);
        ...
        return(
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} contentContainerStyle={[AppStyles.row, AppStyles.paddingRight, AppStyles.paddingLeftSml]}>
                {
                  mustTips.reverse().map((item, index) => {
                      return(
                          <TouchableOpacity
                              key={index}
                              onPress={() => { this.textInput.blur() ; this.setState({sendAmount: item.amount}) }}
                              style={[
    ...
    render() {
        ...
        switch (step){
            case 'waiting':
                return(
                    <Animated.View
                        style={[styles.container, { flexGrow: 1, paddingBottom: this.KeyboardHeight}]}
                    >
                        <View style={[AppStyles.flex6, AppStyles.centerAligned]}>
                            <Text style={[AppStyles.h5, AppStyles.strong, AppStyles.textCenterAligned]}>Destination</Text>
                            <Text style={[AppStyles.baseText, AppStyles.textCenterAligned]}>Select a contact or scan QR code</Text>
                            <Spacer size={20}/>
                            {this.renderPicker()}

                            <Spacer size={40}/>

                            <Text style={[AppStyles.h5,AppStyles.strong, AppStyles.textCenterAligned]}>Set the amount you want to tip</Text>
                ...
                return(
                    ...
                    >
                        <LottieView
                            ...
                            resizeMode={'contain'}
                        />
                        <Text style={[AppStyles.h5, AppStyles.textCenterAligned, styles.whiteText]}>Sending Tip ...</Text>
                    </LinearGradient>
                );
                ...
                return(
                    ...
                    >
                        <LottieView
                            ...
                            style={{ width: AppSizes.screen.width * 0.50,
                                height: AppSizes.screen.height * 0.50,}}
                            autoPlay
                            loop={false}
                            resizeMode={'cover'}
                        />
            ...
            case 'success':
                return(
                    ...
                    >
                        ...
                        />
                        <Text style={[AppStyles.h5, AppStyles.textCenterAligned, styles.whiteText]}>
                            Successfully Sent { sendAmount.startsWith('.') ?  '0' + sendAmount: sendAmount } XRP {sendTo.network === "internal" ? "" : ( sendTo.network === "discord" ? `to ${sendTo.slug}` : `to ${sendTo.username}`)}
                        </Text>
                    </LinearGradient>
                )
        }
